 ...
 * Note: When the value of the availability column is UNAVAILABLE, only the itemId
 * and availability columns are populated.
 */
class ItemSnapshot extends AbstractModel
{
    /**
    ...
    /**
     * The label of the category of the item. For example: Toys &amp; Hobbies|Action
     * Figures|Comic Book Heroes .
     *
     * @var string
     */
     ...
     * of condition names, see Item Condition IDs and Names. Note: In all eBay
     ...
     * Refurbished', a seller must be pre-qualified by eBay for this feature. Code so
    ...
    /**
     * The price of the item, which can be a discounted price. Note: The price includes
     ...
     * date and duration. For Good 'Tail Cancelled items, no value is returned in this
    ...
    /**
     * This timestamp denotes the date and time the changes for that item were picked
     * up and added to the snapshot feed file. For example, let's say you have a
     * snapshot feed file and also ran the getItem method. When you compare the same
     * item information from the two sources, you see that the price in the getItem
     * method response is different from the price in the snapshot feed file. By
     * knowing the date and time you submitted the getItem method, you can use the
     * itemSnapshotDate data to determine which price is the most current for this
     * item.
     *
     * @var string
     */
    public $itemSnapshotDate = null;
    ...
    /**
     * An enumeration value representing how a buyer is refunded when an item is
    ...
    /**
     * An enumeration value that indicates the party responsible for the return
     * shipping costs when an item is returned. Valid Values: BUYER or SELLER Code so
    ...
    /**
     * Indicates the European energy efficiency rating (EEK) of the item. This field is
     * returned only if the seller specified the energy efficiency rating. The rating
     * is a set of energy efficiency classes from A to G, where 'A' is the most energy
     * efficient and 'G' is the least efficient. This rating helps buyers choose
     * between various models. To retrieve the manufacturer's specifications for this
     * item, when they are available, use the getItem method in the Browse API. The
     * information is returned in the productFicheWebUrl field.
     *
     * @var string
     */
    ...
    /**
     * Status change indicator of the listing. Values: PRICE_CHANGE QUANTITY_CHANGE
     * TITLE_CHANGE DELETED NEW ENDED.
     *
     * @var string
     */
